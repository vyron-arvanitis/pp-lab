...
\subsection{Flat MLP and Masking} \label{sec:method_flatmlp}
...
To assess the effect of padding‐aware inputs, we compare two variants of this flat MLP:
\begin{itemize}
...
We trained the flat MLP for both cases on 1000 events and 10 epochs, observing the following: 
...
Given these results, we decided to exclude the flat MLP from further testing and optimization and focused on architectures that better exploit the symmetries and relational structure of particle-set data.
...
\begin{enumerate}
  \item \textbf{Embedding extraction.} We loaded the trained configuration for our optimal model (\texttt{optimal\_model\_cylindrical} described in the next section) and read out the weight tensor of the embedding layer.
...
\item \textbf{Cosine‐similarity matrix.}
  ...
  full pairwise cosine matrix \(S\). We recorded both the full–matrix and off‐diagonal statistics.
