...
\begin{itemize}
    ...
    \item Are there any redundant features in the dataset that could be removed? (subsection \ref{sec:Unnecessary Features})
...
This model introduces two additional configuration arguments:
\begin{itemize}
...
\item  The final output module consists of two linear layers separated by a ReLU activation (i.e., a small MLP).
\end{itemize}
...
To enable coordinate transformation, the function \texttt{preprocess} in \texttt{utils.py} takes an additional argument, 
...
\begin{itemize}
  ...
  \item \textbf{Modest performance.} Even with normalization, dropout or adding PDG ids features, accuracy plateaus around $60-65\%$, far below our best models.
\end{itemize}
...
\subsection{PDG Embedding Layer Inspection} \label{sec:embedding_study}
...
\item \textbf{Cosine‐similarity matrix.}
  ...
  \item \textbf{Visualization.} We produced a 2D t‐SNE projection of the embeddings (Figure~\ref{fig:pdg_tsne}), highlighting six common species (\(\mu^-, \gamma, \pi^0, \pi^+, K^+, p\)).  Finally, we plotted a histogram of the off‐diagonal cosine similarities (Figure~\ref{fig:pdg_hist}) to show the broad distribution from \(-1\) to \(+1\).
\end{enumerate}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{chapters/methodology/pdg_embeddings_tsne.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{chapters/methodology/pdg_embedding_hist.png}
