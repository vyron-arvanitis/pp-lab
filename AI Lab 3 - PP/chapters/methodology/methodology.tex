...
The dataset we are working with consists of simulated particle collision events, with 
each event containing multiple particles. Each particle is described by a fixed number of features.
These features are:
...
Each event is represented as as a tensor of shape \texttt{(num\_particles, num\_features)}. 
Since the number of particles can vary between events, padding is applied to ensure consistent input dimensions across batches. 
These padded values are properly masked during training to prevent them from affecting the model.
...
\begin{figure}[h]
 \centering
    ...
    \caption{Validation accuracy and loss for different experiments. The horizontal dashed lines indicate the best performance achieved with the baseline model.}
