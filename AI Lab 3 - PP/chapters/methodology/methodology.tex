...
\begin{itemize}
    \item Does it help to increase the number of linear layers? Which sequence of GCN and Linear layers works best? \ref{sec:Varying_layers_in_the_deepset_model}
    ...
    \item Do we need to transform the values for the particle features? \ref{sec:Nomralize features}
    \item Are there any redundant features in the dataset that could be removed? \ref{sec:Unecessary Features}
    \item Could one make use of some (approximate) symmetries in the data? \ref{sec:Exploiting Rotational Symmetry}
...
the best model was \texttt{DS\_4\_GCN\_123}: 4 hidden layers, with GCNs at the input, second and third layer.
...
more compact format using cylindrical coordinates, while discarding unnecessary information by neglecting the angular coordinate. To test this hypothesis, the models \texttt{deepset}, \texttt{deepset\_gcn}, and \texttt{deepset\_combined} were trained using both Cartesian and cylindrical coordinate represenatations. 
...
\begin{table}[h]
    ...
    \begin{tabular}{l|cc|cc}
        ...
        \hline
        \texttt{deepset}& $0.6294$ & $63.88~\%$ & $0.6219$&$64.69~\%$ \\
        \texttt{deepset\_gcn}&$0.6235$&$64.79~\%$&$0.6119$&$66.21~\%$ \\
        \texttt{deepset\_combined}&$0.4767$&$77.04~\%$&$0.4727$ &$77.43~\%$\\
        \hline
    \end{tabular}
\end{table}
...
Specifically, the validation accuracy increases by $0.81\%$ for \texttt{deepset}, $1.42\%$ for \texttt{deepset\_gcn}, and $0.39\%$. 
...
It produces negative outputs that push mean activations closer to zero, improving learning dynamics. The non-zero gradient for negative inputs enhances backpropagtation compared to ReLU
...
ELU to find a suitable activation function for the optimal model.
\begin{table}[h]
    \centering
    ...
    \begin{tabular}{l|cc|cc|cc}
        ...
        \hline
        \texttt{deepset} & 0.640 & 63.6 & 0.639 & 64.1 & 0.629 & 63.1 \\
        \texttt{deepset\_combined} & 0.771 & 77.5 & 0.773 & 77.3 & 0.765 & 77.0 \\
        \texttt{deepset\_gcn} & 0.658 & 66.0 & 0.660 & 66.1 & 0.637 & 64.0 \\
        \hline
    \end{tabular}
\end{table}
