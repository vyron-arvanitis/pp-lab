...
MLPs are universal function approximators: by stacking several layers one can approximate arbitrarily complex mappings from its fixed‐length input vector to the desired output.
...
Because an MLP operates on a flat vector of features, it requires a predetermined input size and implicitly assumes an ordering of those inputs.  In tasks where the data are naturally sets or graphs (e.g.\ collections of particles), this fixed‐order assumption provides no direct mechanism to enforce permutation invariance or to leverage relational structure, as opposed to Deep Sets or GCN. 
...
The aforementioned property makes the transformer architecture highly suitable for
